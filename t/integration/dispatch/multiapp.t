...
subtest 'uri_for_route rejects a route parameter of 0 (known bug F1)' => sub {
    my $test = multi_app('MultiUri');

    # Current behavior, pinned deliberately: the value is tested for truth, so
    # a defined-but-false parameter is reported as "not provided". See F1 in
    # paad/test-roadmap/test-roadmap-findings.md. When that is fixed this test
    # goes red -- update it to expect http://localhost/item/0.
    my $content = $test->request( GET '/uri/zero' )->content;
    like( $content, qr/^DIED/, 'a route parameter of 0 dies' );
    like( $content, qr/uses the parameter 'id', which was not provided/,
        'and claims the parameter was not provided, though it was' );
};
