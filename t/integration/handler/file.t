...
subtest 'a null byte in a static path is survived, not served' => sub {
    ...
    # Asserted rather than silenced: the static handler's file-existence check
...
subtest 'Dancer2::Handler::File does not contain ../ paths (known bug)' => sub {
    ...
    # is still inside public_dir - unlike send_file, which does exactly that
    # check at Dancer2/Core/App.pm:1182, and unlike Plack::App::File on the
    # default static path, both of which answer 403 for the same request.
    #
    # Deliberately not fixed here: these tests pin current behavior. When the
    ...
    is( $res->code, 200,
        'BUG: a ../ path escaping public_dir is served with 200' );
    is( $res->content, 'SECRET',
