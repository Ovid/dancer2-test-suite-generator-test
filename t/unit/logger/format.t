...
subtest '%D is documented but not implemented (known bug)' => sub {
    ...
    # through to the unknown-code path: it warns and renders '-'.
    #
    # Anyone who followed the documentation gets a Carp warning on every single
    # log line, which is the part that makes this worth recording rather than
    # shrugging at.
    #
    # Deliberately not fixed; filed as F2 in
    # paad/test-roadmap/test-roadmap-findings.md. Implementing %D - or removing
    # it from the POD - turns this subtest red.

    my ( $line, $warnings ) = render('%D');

    is( $line, '-', 'BUG: %D renders as "-" rather than a timer' );
    is( scalar @$warnings, 1, 'BUG: and warns, once per log line' );
    like( $warnings->[0], qr/%D not supported/,
        'BUG: with the same message as a code that was never documented' );

    # The comparison that makes the point: a documented code and an
    # undocumented one are treated identically.
    my ( $undocumented ) = render('%Z');
    is( $line, $undocumented,
        'BUG: %D behaves exactly like the undocumented %Z' );
};
